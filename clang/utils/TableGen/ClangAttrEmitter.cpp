...
public:
  ...
  std::string getIsOmitted() const override {
    return "!((is" + getLowerName().str() + "Expr && " + getLowerName().str() +
           "Expr) || (!is" + getLowerName().str() + "Expr && " +
           getLowerName().str() + "Type))";
  }
...
public:
  ...
  void writeAccessors(raw_ostream &OS) const override {
    ...
    OS << "  " << IteratorType << " " << BeginFn << " const {" << " return "
       << ArgName << "; }\n";
    OS << "  " << IteratorType << " " << EndFn << " const {" << " return "
       << ArgName << " + " << ArgSizeName << "; }\n";
    OS << "  unsigned " << getLowerName() << "_size() const {" << " return "
       << ArgSizeName << "; }\n";
  ...
  void writeTemplateInstantiationArgs(raw_ostream &OS) const override {
    ...
    OS << "A->" << getLowerName() << "_begin(), " << "A->" << getLowerName()
       << "_size()";
  }
  ...
  void writeConversion(raw_ostream &OS, bool Header) const {
    if (Header) {
      OS << "  static bool ConvertStrTo" << shortType << "(StringRef Val, "
         << fullType << " &Out);\n";
      OS << "  static const char *Convert" << shortType << "ToStr(" << fullType
         << " Val);\n";
      return;
    }
...
public:
  ...
  void writeASTVisitorTraversal(raw_ostream &OS) const override {
    OS << "  if (!" << "getDerived().TraverseStmt(A->get" << getUpperName()
       << "()))\n";
    OS << "    return false;\n";
  }
  ...
  void writeTemplateInstantiation(raw_ostream &OS) const override {
    ...
    OS << "        ExprResult " << "Result = S.SubstExpr(" << "A->get"
       << getUpperName() << "(), TemplateArgs);\n";
...
public:
  ...
  void writeTemplateInstantiationArgs(raw_ostream &OS) const override {
    OS << "tempInst" << getUpperName() << ", " << "A->" << getLowerName()
       << "_size()";
  }
  ...
  void writeHasChildren(raw_ostream &OS) const override {
    OS << "SA->" << getLowerName() << "_begin() != " << "SA->" << getLowerName()
       << "_end()";
  }
};
...
// FIXME
/*
static void GenerateHandleStmtAttribute(const Record &Attr, raw_ostream &OS) {
  // Only generate if Attr can be handled simply.
  if (!Attr.getValueAsBit("SimpleHandler"))
    return;

  // Generate a function which just converts from ParsedAttr to the Attr type.
  OS << "Attr* handleStmtAttribute(Sema &S, Stmt *St,";
  OS << "const ParsedAttr &Attr) const override {\n";
  OS << "  return ::new (S.Context) " << Attr.getName();
  OS << "Attr(S.Context, Attr);\n";
  OS << "}\n\n";
}
*/
...
void EmitClangAttrParsedAttrImpl(RecordKeeper &Records, raw_ostream &OS) {
  ...
  for (auto I = Attrs.begin(), E = Attrs.end(); I != E; ++I) {
    ...
    // FIXME
    // GenerateHandleStmtAttribute(Attr, OS);
...
GetAttributeHeadingAndSpellings(const Record &Documentation,
                                const Record &Attribute, StringRef Cat) {
  ...
  if (Heading.empty()) {
    ...
    if (Spellings.size() == 1)
      ...
      std::set<std::string> Uniques;
      for (auto I = Spellings.begin(), E = Spellings.end(); I != E; ++I) {
        std::string Spelling =
